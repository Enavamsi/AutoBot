 ...
 * IMU          : Adafruit BNO085 (BNO08x) over I2C, pins set by IMU_SDA_PIN/IMU_SCL_PIN
 *
 ...
 * instead of begin_I2C() in setup(). This node also detects internal sensor
 * resets (bno08x.wasReset()) and automatically re-enables reports so it
 * recovers instead of silently going quiet.
 *
...
// ==========================================================
#define WHEEL_RADIUS_M        0.05f   // TODO: measure your wheel radius (m)
...
// --- BNO085 IMU ---
// Explicit "-1" = no hardware reset pin wired up (matches the known-working
// standalone test sketch). If you DO have the reset pin wired, pass that
// GPIO number instead.
...
// (called at boot, and again any time the sensor internally resets)
// ==========================================================
void setImuReports() {
...
//
// Also checks wasReset(): the BNO085 occasionally resets itself
// internally (brownout, I2C glitch, etc). When that happens all
// previously-enabled reports are cleared sensor-side, so if we don't
// re-enable them here, /imu/data will silently stop updating forever.
// ==========================================================
void pollImu() {
  if (bno08x.wasReset()) {
    setImuReports();
...
void setup() {
  ...
  // --- BNO085 IMU (I2C, explicit pins, matching the known-working test sketch) ---
